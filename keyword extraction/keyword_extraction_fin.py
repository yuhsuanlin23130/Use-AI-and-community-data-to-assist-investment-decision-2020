...
for indexxx, riseordown in enumerate(['看漲','看跌']):     
    for index, row in df.iloc[:].iterrows():  
        ...
        title = str(row["title"])
        ########################################
        if ('聯發科' in text or '蔡明介' in text) and 'NYSE' not in text:      
            #cnt = 0
...
for indexxx, riseordown in enumerate(['看漲','看跌']):   
    ...
    df_col = ['gram','TF','DF','TF_IDF','all_TF','all_DF','all_TF_IDF','expected_TF','expected_DF','DF_chisquare','MI','Lift']    
    ...
    for i in range(len(ngrm)):
        ...
        for gram,(TF,DF) in class_ngrm[i]:         
            ...
            grm = ([gram,TF,DF,TF_IDF,all_TF,all_DF,all_TF_IDF,expected_TF,expected_DF,DF_chisquare,MI,Lift])
            sheet1.append(grm)   
